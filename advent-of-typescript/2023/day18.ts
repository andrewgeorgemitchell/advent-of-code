...
// Solution
type Count<
  TSack extends string[],
  UItem extends TSack[number] | string,
  VStore extends UItem[] = [],
> = TSack extends [infer Curr, ...infer Rest]
  ? Rest extends string[]
    ? Curr extends UItem
      ? Count<Rest, UItem, [...VStore, Curr]>
      : Count<Rest, UItem, VStore>
    : VStore['length']
  : VStore['length']
